...
Cache::Cache(MemorySystem *gms, const char *section, const char *name)
  ...
  ,writeMiss2("%s:writeMiss2", name)
  ,readMiss("%s:readMiss", name)
  ,readMiss2("%s:readMiss2", name)
//  ,nAccess2("%s:nAccess2", name)
  ...
  //,maxThreshold2 ("%s:maxThreshold2", name)

  ,switchActiveCounter("%s:switchActiveCounter", name)
  ,switchDrowsyCounter("%s:switchDrowsyCounter", name)
  ,cyclesLinesActive("%s:cyclesLinesActive", name)
  ,cyclesLinesDrowsy("%s:cyclesLinesDrowsy", name)
...
void Cache::access(MemRequest *mreq) 
{
  ...
  case MemReadW:
  case MemRead:    {read(mreq); makeLinesDrowsy();}       break;
  case MemWrite:   {write(mreq); makeLinesDrowsy();}      break;
  case MemPush:    {pushLine(mreq); makeLinesDrowsy();}   break;
  default:         {specialOp(mreq); makeLinesDrowsy();}  break;
  }
}
...
void Cache::makeLinesDrowsy ()
{
  long long nAccess2 = nAccesses[0]->getValue() ;
  int numLinesActive = 0;
  int numLinesDrowsy = 0;
  int numLines = cacheBanks[0]->getNumLines();
  int k;
  currentclocktick = globalClock;
  Time_t cyclesElapsed;
  
  numLinesActive = cacheBanks[0]->getNumLines();
  ...
  cyclesElapsed =  currentclocktick - prevclocktick;
  long long data2;
  prevclocktick = currentclocktick;
  

  data2 = cyclesElapsed * numLinesActive;
 
  cyclesLinesActive.setValueInc(data2);
  
  data2 = numLinesDrowsy;
  cyclesLinesDrowsy.setValue(data2);
 

  /*if (nAccess2 %  100 == 0){
    for (k = 0; k < (numLines) ; k++) {
      Line *l = cacheBanks[0]->getPLine(k); 
      if(l->getMaxThreshold() > 990 && l->isLineOn() == 2 ){
        l->switchOffLine();
        switchDrowsyCounter.inc();
      }
    }
  }*/
...
void Cache::doRead(MemRequest *mreq)
{
  ...
  if (l->isLineOn() == 1){
    l->switchOnLine(); 
    switchActiveCounter.inc();
    readHit.inc();
    l->maxThreshold--;
    l->incReadAccesses();

    mreq->goUp(hitDelay+1);
    return;
  }
  readHit.inc();
  l->maxThreshold--;
