...
private:
 ...
 // int32_t isLineActive2; //0 - inactive, 1 - active
public:
  CState() {
    valid = false;
    dirty = false;
...
protected:
 ...
 // GStatsCntr nAccess2;
  ...
  GStatsCntr readMiss2;
  GStatsCntr writeMiss2;
  GStatsCntr switchingOffCounter;
  GStatsCntr switchingOnCounter;
  GStatsCntr cyclesLinesOn;

  GStatsCntr switchActiveCounter;  //Switching drowsy to active
  GStatsCntr switchDrowsyCounter;  //Switching active to drwosy

  GStatsCntr cyclesLinesActive;  // how many lines are in active mode during specified time period
  GStatsCntr cyclesLinesDrowsy;  // how many lines are in drowsy mode during same time period

  // END Statistics

  Time_t currentclocktick;
  Time_t prevclocktick;
...
public:
  ...
  virtual void switchOffWays ();
  virtual void makeLinesDrowsy ();
